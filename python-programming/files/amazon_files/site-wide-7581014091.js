...
if(opt.twoPane){width=Math.max(width,649);
...
lookup(opt,"pe");
if(opt.twoPane==false){searchSuggest=new SS(searchBox,opt,'<div id="srch_sggst" style="display:none"/>',{afterCreate:resizeHandler,onWindowResize:resizeHandler,beforeShow:resizeHandler});
}else{searchSuggest=new SS(searchBox,opt,'<div id="srch_sggst" class="two-pane" style="display:none"/>',{afterCreate:twoPaneSetPosition,beforeHtmlChange:twoPaneDestroyFlyout,beforeShow:twoPaneSetPosition,afterShow:function(flyout){twoPaneSetPosition.call(this,flyout);
...
}}function twoPaneSetPosition(flyout){if(flyout){return;
...
},isTwoPane:function(){return opt.twoPane===1;
