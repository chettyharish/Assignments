...
}if(imageBlockData&&imageBlockData.litbData&&imageBlockData.litbData.hasLitb&&imageBlockData.litbData.litbReftag){sitbLogoImage.removeClass("a-hidden");
sitbLogoImage.click(addLitb);
canvas.click(addLitb);
...
var canvasLeft=(imgContainer.outerWidth()-frontImage.outerWidth())/2,canvasTop=imgContainer.outerHeight()-frontImage.outerHeight();
canvas.css("top",canvasTop).css("left",canvasLeft);
repositionLogo(canvasTop,canvasLeft,canvasWidth);
...
}function repositionLogo(canvasTop,canvasLeft,canvasWidth){sitbLogo.css("top",canvasTop+imgBlockMargin).css("left",canvasLeft+canvasWidth-sitbLogo.width());
...
}function isIEBrowser(){return $.browser.msie||!!(navigator.userAgent.match(/Trident/));
}});
...
var buyDealSection="buyDealSection";
var isDealAvailable=$("#"+buyDealSection).length;
...
}if(isDealAvailable&&typeof UDPDealView!="undefined"){$("#gbld_header_message").show();
if(bbClicked==buyDealSection){UDPDealView.prototype.onSelectLDBuybox();
...
galleryOpenLogged=1;
}}function setLogger(){amznJQ.available("DPClientLogger",function(){if(typeof window.DPClientLogger!="undefined"){DPCL=new window.DPClientLogger.ImpressionLogger("dpbxapps","bxapps-atfMarker",true,true);
}});
}function setSelectedImage(id){selectedImageId=id;
...
}}}function setDims(){height=popoverWrapper.height();
...
imageInner.css("height",contentHeight).css("width",contentWidth).data("height",contentHeight).data("width",contentWidth);
}}function intializeContent(){if(popoverWrapper.length>0){setDims();
updateImage();
addThumbs();
zoomOptions=getZoomOptions();
zoomer.setImageData(zoomOptions);
zoomer.attachZoomEvents();
}}function addThumbs(){if(numThumbs>0){for(var i=0;
i<numThumbs;
i++){var thumbId="ig-thumb-"+i,selected=thumbId==selectedImageId?"selected":"";
...
}}}function setImageDims(){var width=images[selectedImageId].dimensions[0],height=images[selectedImageId].dimensions[1];
...
}function updateImage(){if(images[selectedImageId]){setImageDims();
image.attr("src",images[selectedImageId].mainUrl);
}}function getImageFitClass(imageWidth,imageHeight){var containerAspectRatio=imageInner.data("width")/imageInner.data("height"),imageAspectRatio=imageWidth/imageHeight,fitClass="image-stretch-vertical";
if(imageAspectRatio>containerAspectRatio){fitClass="image-stretch-horizontal";
}return fitClass;
}function getZoomOptions(){var options={};
if(images[selectedImageId]){options.containerId="igInner";
options.imageId="igImage";
options.imageWidth=images[selectedImageId].dimensions[0];
options.imageHeight=images[selectedImageId].dimensions[1];
options.DPCL=DPCL;
}return options;
...
if(!currObj.hasClass("selected")&&newImageData){setSelectedImage(id);
changeThumb(currObj);
zoomer.detachEvents();
attachImageOnload();
updateImage();
var zoomOptions=getZoomOptions();
zoomer.setImageData(zoomOptions);
if(DPCL&&!thumbInteractionLogged){DPCL.logImpression("ma-img-gallery-thumb");
thumbInteractionLogged=1;
}}});
}}function changePopoverDims(){var viewport=A.viewport(),ratio=0.75;
content.closest(".a-popover-modal").css("width",viewport.width*ratio).css("height",viewport.height*ratio);
}function updateOrientation(){changePopoverDims();
setDims();
zoomer.detachEvents();
setImageDims();
var zoomOptions=getZoomOptions();
zoomer.setImageData(zoomOptions);
zoomer.attachZoomEvents();
}function bindOrientationChange(){if(A.capabilities.orientation){A.on("orientationchange",function(){updateOrientation();
});
A.on("a:popover:afterHide:imageGallery",function(){A.off("a:popover:afterHide:imageGallery");
A.on("a:popover:beforeShow:imageGallery",function(){updateOrientation();
});
});
...
function ImageZoomer(){var zoomer={},containerId=null,imageId=null,imageWidth=0,imageHeight=0,containerObj=null,imageObj=null,hasXZoom=false,hasYZoom=false,zoomInteractionLogged=0,DPCL,isTouch=A.capabilities.touch;
...
function touchZoomEngage(){imageObj.bind("touchstart.ig touchmove.ig",function(event){positionImage(event);
imageObj.unbind("touchstart.ig touchmove.ig");
imageObj.bind("touchmove.ig",function(event){positionImage(event);
});
imageObj.bind("touchend.ig",function(event){zoomer.zoomOut();
imageObj.unbind("touchstart.ig touchmove.ig touchend.ig");
touchZoomEngage();
});
});
}zoomer.attachZoomEvents=function(){if(hasXZoom||hasYZoom){containerObj.addClass("zoomable");
if(isTouch){touchZoomEngage();
}else{imageObj.click(function(event){if(DPCL&&typeof DPCL.logImpression=="function"&&!zoomInteractionLogged){DPCL.logImpression("ma-img-gallery-zoom");
...
imageObj.css("top","auto").css("left","auto");
};
...
}function positionImage(event){if(isTouch){event.preventDefault();
}var pos=getZoomPosition(event);
zoomer.zoomIn(pos);
