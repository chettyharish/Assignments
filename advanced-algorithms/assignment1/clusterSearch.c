...
int findMin(int *leaderDistances, int numClusters, int least) {
	int i = 0, min = INT_MAX, pos = -1;

	for (i = 0; i < numClusters; i++) {
		if (abs(leaderDistances[i]) < min) {
			if (leaderDistances[i] >= 0 && leaderDistances[i] < least) {
				min = leaderDistances[i];
				pos = i;
			}

			if (leaderDistances[i] < 0) {
				min = leaderDistances[i];
				pos = i;
			}
		}
...
int main(int argc, char *argv[]) {
	...
	int tempcount[totalQuery];
	int count[totalQuery];
	int count2[totalQuery];
	int resolved[totalQuery];
	int realmins[totalQuery];
...
#pragma openmp parallel for
	for (query = 0; query < totalQuery; query++) {
		...
		for (i = 0; i < numClusters; i++) {
			int pos = findMin(leaderDistances, numClusters, least);
